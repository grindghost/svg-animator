...
"scroll-loop": {
  "defaultSpeed": "3.0",
  "defaultSpeedSlider": true,
  "apply": function(element, p = {}, speed) {
    const actualSpeed = speed || parseFloat(this.defaultSpeed) || 3;
    ...
    const wrapperId = `scrollLoopWrapper-${element.id}`;
    let wrapper = document.getElementById(wrapperId);

    // --- If wrapper already exists, just update animation speed ---
    if (wrapper) {
      return;
      // Only update, never move nodes
      wrapper.querySelectorAll("animateTransform.scroll-loop-anim").forEach(anim => {
        // Use setAttribute, no appendChild, no replaceChild
        anim.setAttribute("dur", `${actualSpeed}s`);
      });
      return; // ✅ stop here, DOM untouched
    }

    // --- Create wrapper if missing ---
    wrapper = document.createElementNS("http://www.w3.org/2000/svg", "g");
    wrapper.classList.add("scroll-loop-wrapper");
    wrapper.setAttribute("id", wrapperId);
    element.parentNode.insertBefore(wrapper, element);
    wrapper.appendChild(element);

    // Freeze bbox for clipping
    const bbox = element.getBBox();
    const x = bbox.x, y = bbox.y, w = bbox.width, h = bbox.height;
    const offset = h;

    // --- Build clipPath once ---
    const clipId = `scrollClip-${element.id}`;
    ...
    // --- Helper: create animated copy ---
    function makeAnimatedCopy(baseNode, startY) {
      const copy = baseNode.cloneNode(true);

      // 🚨 strip out any accidental animateTransform
      copy.querySelectorAll("animateTransform").forEach(a => a.remove());

      copy.removeAttribute("id");
      copy.classList.add("scroll-clone");

      const animate = document.createElementNS("http://www.w3.org/2000/svg", "animateTransform");
      animate.classList.add("scroll-loop-anim"); // 🔑 mark for updates
      animate.setAttribute("attributeName", "transform");
      animate.setAttribute("type", "translate");
      animate.setAttribute("from", `0 ${startY}`);
      animate.setAttribute("to", `0 ${startY + offset}`);
      animate.setAttribute("dur", `${actualSpeed}s`);
      animate.setAttribute("repeatCount", "indefinite");

      copy.appendChild(animate);
      return copy;
    }

    // --- Animate original ---
    const origAnim = document.createElementNS("http://www.w3.org/2000/svg", "animateTransform");
    origAnim.classList.add("scroll-loop-anim"); // 🔑 mark for updates
    origAnim.setAttribute("attributeName", "transform");
    origAnim.setAttribute("type", "translate");
    origAnim.setAttribute("from", "0 0");
    origAnim.setAttribute("to", `0 ${offset}`);
    origAnim.setAttribute("dur", `${actualSpeed}s`);
    origAnim.setAttribute("repeatCount", "indefinite");
    element.appendChild(origAnim);

    // --- Add single clone above ---
    wrapper.appendChild(makeAnimatedCopy(element, -offset));
  }
}
