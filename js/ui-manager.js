...
function createTreeViewItem(parent, element, depth = 0) {
    ...
    if (element.tagName === 'clipPath') {
        ...
        // Add element ID display if it exists and is user-provided
        // Skip SVG root and clipPath elements as they're structural and don't need ID display
        if (element.id && element.id !== '' && isUserProvidedID(element.id) && 
            element.tagName !== 'svg' && element.tagName !== 'clipPath') {
            const idSpan = document.createElement('span');
            idSpan.className = 'tree-item-id';
            idSpan.style.opacity = '0.7';
            idSpan.style.marginLeft = '4px';
            idSpan.textContent = `(${element.id.length > 20 ? element.id.substring(0, 20) + '...' : element.id})`;
...
// Check if an ID is user-provided (not auto-generated by the app)
function isUserProvidedID(id) {
    if (!id) return false;
    
    // Auto-generated IDs follow the pattern: id-xxxxxxxxx (9 random chars)
    // They start with 'id-' followed by exactly 9 alphanumeric characters
    const autoGeneratedPattern = /^id-[a-z0-9]{9}$/;
    
    return !autoGeneratedPattern.test(id);
}
